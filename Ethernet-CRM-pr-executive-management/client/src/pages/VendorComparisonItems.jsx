...
export default function VendorComparisonItems() {
  ...
  useEffect(() => {
    ...
    const load = async () => {
      ...
      try {
        ...
        if (!result?.success) {
          setError(result?.error || "Unable to load vendor comparison items.");
          setComparisons([]);
          return;
        }
        ...
        if (active) {
          setError(err?.message || "Unable to load vendor comparison items.");
          setComparisons([]);
        }
      } finally {
    ...
    <div className="space-y-6">
      <section className="rounded-2xl border border-border bg-gradient-to-r from-sky-50 via-white to-emerald-50 p-6 md:p-8 dark:from-slate-900 dark:via-slate-900 dark:to-slate-800/70">
        <div className="flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
          <div>
            <h1 className="text-3xl font-bold tracking-tight">VC Items</h1>
            <p className="mt-1 text-sm text-muted-foreground">
              {vendorName ? `Items used for ${vendorName}` : "All vendor comparison items for the project."}
            </p>
          </div>
          <div className="flex flex-col gap-2 sm:flex-row">
            ...
            >
              <ArrowLeft className="mr-2 h-4 w-4" /> Back to VC Preview
            </Button>
            <Button onClick={() => navigate(routeProjectId ? `/${routeProjectId}/vendor-comparison` : projectId ? `/${projectId}/vendor-comparison` : "/vendor-comparison")}>
              Open VC List
            </Button>
          </div>
        </div>
      ...
      <Card className="border-0 shadow-sm ring-1 ring-border/50 overflow-hidden">
        <CardHeader className="border-b bg-muted/20">
          ...
          <CardDescription>Search vendor comparison items by comparison id, PR no, or item name.</CardDescription>
        </CardHeader>
        <CardContent>
          <div className="relative">
            <Search className="pointer-events-none absolute left-3 top-3.5 h-4 w-4 text-muted-foreground" />
            <Input value={query} onChange={(event) => setQuery(event.target.value)} className="pl-9" placeholder="Search VC items..." />
          </div>
        </CardContent>
      </Card>
      ...
      {loading ? (
        <Card className="border-0 shadow-sm ring-1 ring-border/50 overflow-hidden">
          <CardContent className="flex items-center justify-center py-16 text-muted-foreground">
            <Loader2 className="mr-2 h-4 w-4 animate-spin" /> Loading VC items...
          </CardContent>
        </Card>
      ) : null}
      ...
      {!loading && !error && filteredComparisons.length === 0 ? (
        <Card className="border-0 shadow-sm ring-1 ring-border/50 overflow-hidden">
          <CardContent className="py-16 text-center text-muted-foreground">No vendor comparison items found.</CardContent>
        </Card>
      ) : null}
      ...
      {filteredComparisons.map((entry) => {
        ...
        return (
          <Card key={`vc-${comparisonId || Math.random()}`} className="border-0 shadow-sm ring-1 ring-border/50 overflow-hidden">
            <CardHeader className="border-b bg-muted/20">
                ...
                <Table>
                  ...
                  <TableBody>
                    {entry.pricelist.map((row, index) => (
                      <TableRow key={`${comparisonId || "vc"}-${index}`}>
