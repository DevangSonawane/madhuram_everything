...
// ─── Logo injector (Lodha) ───────────────────────────────────────────────────
/**
 * Injects the Madhuram logo into the Lodha INV sheet using raw XML.
 * The logo file is loaded from `/assets/madhuram-logo-app.jpeg`.
 */
async function injectLodhaLogo(wb, sheetName) {
  if (!EMBED_LOGO) return;
  ...
  const logoUrl = new URL(`${normalizedBaseUrl}assets/madhuram-logo-app.jpeg`, window.location.origin).toString();

  let imgBytes;
  try {
    imgBytes = await fetchBytes(logoUrl);
  } catch {
    // Fallback (dev servers / different base URL setups)
    try {
      imgBytes = await fetchBytes("/assets/madhuram-logo-app.jpeg");
    } catch {
      return;
    }
  }
  ...
  ws["!drawing"] = {
    _data: imgBytes,
    // Place a small logo at the top-right-ish area of the sheet.
    _xml: `<?xml version="1.0" encoding="UTF-8" standalone="yes"?>
<xdr:wsDr xmlns:xdr="http://schemas.openxmlformats.org/drawingml/2006/spreadsheetDrawing" xmlns:a="http://schemas.openxmlformats.org/drawingml/2006/main">
  <xdr:twoCellAnchor editAs="oneCell">
    <xdr:from><xdr:col>9</xdr:col><xdr:colOff>0</xdr:colOff><xdr:row>0</xdr:row><xdr:rowOff>0</xdr:rowOff></xdr:from>
    <xdr:to><xdr:col>12</xdr:col><xdr:colOff>0</xdr:colOff><xdr:row>2</xdr:row><xdr:rowOff>0</xdr:rowOff></xdr:to>
    <xdr:pic>
      <xdr:nvPicPr><xdr:cNvPr id="2" name="Picture 1"/><xdr:cNvPicPr><a:picLocks noChangeAspect="1"/></xdr:cNvPicPr></xdr:nvPicPr>
      <xdr:blipFill><a:blip xmlns:r="http://schemas.openxmlformats.org/officeDocument/2006/relationships" r:embed="rId1"/><a:stretch><a:fillRect/></a:stretch></xdr:blipFill>
      <xdr:spPr><a:xfrm><a:off x="6200000" y="0"/><a:ext cx="1600000" cy="700000"/></a:xfrm><a:prstGeom prst="rect"><a:avLst/></a:prstGeom></xdr:spPr>
    </xdr:pic>
    <xdr:clientData/>
  </xdr:twoCellAnchor>
</xdr:wsDr>`,
...
// ─── Lodha Full Package (WIP) ───────────────────────────────────────────────
// Builds the wider Lodha RA package sheets (CUMM BOQ etc.) from RA form data.
// Note: This keeps the existing INV sheet as-is and adds extra sheets.

export function buildCummBOQSheet(boqItems, formData) {
  const header = withCommonCompanyHeader(formData?.header || {});
  const meta = formData || {};

  const items = Array.isArray(boqItems) ? boqItems : Array.isArray(meta?.boq_items) ? meta.boq_items : [];
  const cells = {};
  const merges = [];

  const TITLE = { font: { bold: true, sz: 12, name: "Calibri" }, alignment: { horizontal: "center", vertical: "center" } };
  const H = { font: { bold: true, sz: 10, name: "Calibri" }, alignment: { horizontal: "left", vertical: "center" } };
  const TH = {
  ...
  const GREEN = { fill: { patternType: "solid", fgColor: { rgb: "E2EFDA" } }, font: { bold: true, sz: 9, name: "Calibri" } };

  // Header
  cells["A1"] = { v: header.company_name || " ", t: "s", s: TITLE };
  merges.push({ s: { r: 0, c: 0 }, e: { r: 0, c: 13 } });
  cells["A2"] = { v: header.company_address || " ", t: "s", s: H };
  merges.push({ s: { r: 1, c: 0 }, e: { r: 1, c: 13 } });

  const ref = meta?.header?.invoice_number || meta?.invoice_number || "";
  const woNo = meta?.projectWork?.work_order_number || meta?.work_order_number || meta?.header?.work_order_number || "";
  const woDate = meta?.projectWork?.work_order_date || meta?.work_order_date || meta?.header?.work_order_date || "";
  const building = meta?.header?.building_name || meta?.building_name || "";
  const raNo = meta?.header?.ra_number || meta?.ra_number || "";

  cells["A4"] = { v: "CUMM BOQ", t: "s", s: TITLE };
  merges.push({ s: { r: 3, c: 0 }, e: { r: 3, c: 13 } });
  cells["A5"] = { v: `Reference: ${ref}`, t: "s", s: H };
  merges.push({ s: { r: 4, c: 0 }, e: { r: 4, c: 13 } });
  cells["A6"] = { v: `Work Order No: ${woNo}    Work Order Date: ${woDate}    Building: ${building}    RA: ${raNo}`, t: "s", s: H };
  ...
  const NUM = "_ * #,##0.00_ ;_ * \\-#,##0.00_ ;_ * \"-\"??_ ;_ @_ ";

  let r = headerRow; // 0-based row index for next row (after headers)
  const pushItemHeader = (item) => {
    const row = r;
    cells[XLSX.utils.encode_cell({ r: row, c: 0 })] = { v: item.item_no || "", t: "s", s: GREEN };
    cells[XLSX.utils.encode_cell({ r: row, c: 1 })] = { v: item.description || "", t: "s", s: GREEN };
    merges.push({ s: { r: row, c: 1 }, e: { r: row, c: 8 } });
    cells[XLSX.utils.encode_cell({ r: row, c: 2 })] = { v: toNum(item.wo_qty), t: "n", z: NUM, s: GREEN };
    cells[XLSX.utils.encode_cell({ r: row, c: 9 })] = { v: item.uom || "", t: "s", s: GREEN };
    cells[XLSX.utils.encode_cell({ r: row, c: 10 })] = { v: toNum(item.rate), t: "n", z: NUM, s: GREEN };
    r += 1;
  };
  const pushPhase = (label, item, phaseKey, weight) => {
    const row = r;
    const prev = toNum(item[`prev_${phaseKey}_qty`]);
    const curr = toNum(item[`curr_${phaseKey}_qty`]);
    const cumm = prev + curr;
    const rate = toNum(item.rate);
    const prevBoq = prev * weight;
    const currBoq = curr * weight;
    const cummBoq = cumm * weight;
    const prevAmt = prevBoq * rate;
    const currAmt = currBoq * rate;
    const cummAmt = cummBoq * rate;

    cells[XLSX.utils.encode_cell({ r: row, c: 1 })] = { v: label, t: "s" };
    cells[XLSX.utils.encode_cell({ r: row, c: 3 })] = { v: prev, t: "n", z: NUM };
    cells[XLSX.utils.encode_cell({ r: row, c: 4 })] = { v: curr, t: "n", z: NUM };
    cells[XLSX.utils.encode_cell({ r: row, c: 5 })] = { v: cumm, t: "n", z: NUM };
    cells[XLSX.utils.encode_cell({ r: row, c: 6 })] = { v: prevBoq, t: "n", z: NUM };
    cells[XLSX.utils.encode_cell({ r: row, c: 7 })] = { v: currBoq, t: "n", z: NUM };
    cells[XLSX.utils.encode_cell({ r: row, c: 8 })] = { v: cummBoq, t: "n", z: NUM };
    cells[XLSX.utils.encode_cell({ r: row, c: 11 })] = { v: prevAmt, t: "n", z: NUM };
    cells[XLSX.utils.encode_cell({ r: row, c: 12 })] = { v: currAmt, t: "n", z: NUM };
    cells[XLSX.utils.encode_cell({ r: row, c: 13 })] = { v: cummAmt, t: "n", z: NUM };
    r += 1;
  };

  const WEIGHTS = { supply: 0.6, install: 0.25, tc: 0.1, handover: 0.05 };
  items.forEach((item) => {
    pushItemHeader(item);
    pushPhase("Supply @ 60%", item, "supply", WEIGHTS.supply);
    pushPhase("Install @ 25%", item, "install", WEIGHTS.install);
    pushPhase("Testing @ 10%", item, "tc", WEIGHTS.tc);
    pushPhase("Handover @ 5%", item, "handover", WEIGHTS.handover);
  });
  ...
  const rowHeights = { 0: 24, 1: 36, 3: 22, 7: 30 };
  return buildSheet(cells, merges, colWidths, rowHeights);
}

export function createLodhaFullPackage(formData, options = {}) {
  const wb = wb_new();
  const normalizedData = {
    ...(formData || {}),
    header: withCommonCompanyHeader(formData?.header || {}),
  };

  const inv = buildLodhaINVSheet(normalizedData);
  XLSX.utils.book_append_sheet(wb, inv, "INV");

  const cumm = buildCummBOQSheet(normalizedData?.boq_items, normalizedData);
  XLSX.utils.book_append_sheet(wb, cumm, "CUMM BOQ");

  // Placeholders for remaining sheets (structure filled in app-side for now)
  XLSX.utils.book_append_sheet(wb, XLSX.utils.aoa_to_sheet([["CHALLAN SUMMARY"]]), "CHALLAN SUMMARY");
  XLSX.utils.book_append_sheet(wb, XLSX.utils.aoa_to_sheet([["ITR SUMMARY"]]), "ITR SUMMARY");
  XLSX.utils.book_append_sheet(wb, XLSX.utils.aoa_to_sheet([["RATE ANALYSIS"]]), "RATE ANALYSIS");
  XLSX.utils.book_append_sheet(wb, XLSX.utils.aoa_to_sheet([["AMEND BOQ"]]), "AMEND BOQ");
  XLSX.utils.book_append_sheet(wb, XLSX.utils.aoa_to_sheet([["CHECK LIST"]]), "CHECK LIST");
...
export function createExcelInvoice(template, data) {
  ...
  const normalizedData = {
    ...(data || {}),
    header: withCommonCompanyHeader(data?.header || {}),
  };

  if (template === "lodha") {
    const ws = buildLodhaINVSheet(normalizedData);
    XLSX.utils.book_append_sheet(wb, ws, "INV");
  } else if (template === "hiranandani") {
    const ws = buildHiranandaniTAXSheet(normalizedData);
 ...
 */
export async function downloadInvoiceExcel(template, data, filename) {
  console.info("[invoice-excel] Generating excel invoice", { template });
  const wb = createExcelInvoice(template, data);
  if (template === "lodha") {
    await injectLodhaLogo(wb, "INV");
  }
  const invoiceNo = data?.header?.invoice_number || "draft";
  const stamp = new Date().toISOString().replaceAll(":", "-").replaceAll(".", "-");
  const safeTemplate = String(template || "invoice").toLowerCase();
  const name = filename || `Invoice_${safeTemplate}_${invoiceNo}_${stamp}.xlsx`;
  console.info("[invoice-excel] Downloading", { name });
